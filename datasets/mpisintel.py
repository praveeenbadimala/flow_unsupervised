...
def make_dataset(dir, split, dataset_type='clean'):
    ...
    img_dir = os.path.join('training',dataset_type)
    assert(os.path.isdir(os.path.join(dir,img_dir)))

    images = []
    for flow_map in glob.iglob(os.path.join(training_dir,flow_dir,'*','*.flo')):
        flow_map = os.path.relpath(flow_map,os.path.join(training_dir,flow_dir))
        ...
        flow_map = os.path.join('training', flow_map)
